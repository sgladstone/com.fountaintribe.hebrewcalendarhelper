...
function civicrm_api3_all_hebrew_dates_calculate($params) {
  ...
  $table_sql = "SELECT table_name FROM information_schema.tables
    WHERE table_schema = '$cur_schema_name'
      AND table_name = '$yahrzeit_table_name'";

  $table_dao = CRM_Core_DAO::executeQuery($table_sql);
  ...
  if (!$table_exists) {
    throw new API_Exception('Could not find yahrzeit temp table', /*errorCode*/ 1234);
  }
  ...
  $dao = CRM_Core_DAO::executeQuery($sql_cleanup);
  ...
  elseif (isset($rtn_data['error_message_birthday_calcs']) && strlen( $rtn_data['error_message_birthday_calcs'] ) > 0) {
    throw new API_Exception("Error: ".$rtn_data['error_message'], 1234);
  }
  ...
  while ($dao->fetch()) {
    ...
    if(    $is_contact_subtype_good <> "1"){
      ...
      if ($con_sub_result['is_error'] == 0 && $con_sub_result['count'] == 1 ){
        ...
        elseif (is_array( $tmp_contact_subtypes ) == FALSE && strlen( $tmp_contact_subtypes ) > 0 ){
          $tmp = $tmp_contact_subtypes;
          $clean_contact_subtypes[] = $tmp;
        }
        elseif (is_array( $tmp_contact_subtypes)) {
    ...
    elseif( $mourner_observance_preference == '0') {
      $mourner_observance_preference = 0; // Hebrew calendar
      //print "<br>Deceased: ".$deceased_name." mourner pref is Hebrew";
    }
    elseif(  $mourner_observance_preference == '1') {
      $mourner_observance_preference = 1; // English calendar
    ...
    elseif ($deceased_date_before_sunset == '0') {
